...
class KFDataToolSetting(object):
    ...
    def __init__(self,configpath):
        ...
        if setting is not None:
            ...
            self.template_path = trystr(setting, "template_path")
            ...
            self.export_vm3_path = abspath(trystr(setting,"export_vm3_path"))
            self.namespace = trystr(setting,"namespace")
            self.code_type = trystr(setting,"code_type")
            self.data_type = trystr(setting,"data_type")

            logging.info("==>import_kfd_path=%s" % self.import_kfd_path)
            logging.info("==>export_code_path=%s" % self.export_code_path)
            logging.info("==>export_kfd_path=%s" % self.export_kfd_path)
            logging.info("==>export_vm3_path=%s" % self.export_vm3_path)
            ...
            if typeDefTypes is not None:
                ...
                for typeDef in typeDefTypes:
                    ...
                    if pyKFDataType.Is_BaseType(typeid):
