...
def CPP_Any_export(kfddatas, kfdtabel, dirpath, includes, clsname, tmpl,export_api, cppcontainers):
    if len(kfddatas) == 0:
        return
    context = {"datas": kfddatas, "includes": includes, "NS": kfdtabel.kfd_ns, "CLASS":clsname}
    context["export_api"] = export_api
    if cppcontainers is not None and len(cppcontainers) > 0:
        context["containers"] = cppcontainers
    headfilestr = tmpl["h"]
