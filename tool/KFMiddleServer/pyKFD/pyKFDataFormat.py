...
class pyKFDataFormat(object):
    ...
    def read_value(bytearr, skip=False, val=None):
        ...
        if size > 0:
            ...
            if pyKFDataType.Is_UnknowOrBaseType(valueType):
    ...
    def read_base_value(bytearr, valtype, skip=False, value=None):
        ...
        if valtype == pyKFDataType.OT_INT8:
            ...
            if skip:
                ...
                knum2 = pyKFDValue()
                knum2.val = bytearr.read_varuint()
                retval = knum2
        elif valtype == pyKFDataType.OT_VARINT:
            if skip:
                bytearr.read_varint()
            else:
                knum2 = pyKFDValue()
                knum2.val = bytearr.read_varint()
                retval = knum2
    ...
    def read_array_value(bytearr, valtype, skip=False, val=None):
        ...
        if valtype == pyKFDataType.OT_ARRAY:
            ...
            if pyKFDataType.Is_UnknowOrBaseType(oType):
                if skip:
                    i = 0
                    while i < size:
    ...
    def write_base_value(bytearr, dataType, valObject):
        ...
        elif dataType == pyKFDataType.OT_VARINT:
            bytearr.write_varint(valObject.val)
    ...
    @staticmethod
    def varint_write_val(buff, val):
        buff.write_ubyte(pyKFDataType.OT_VARINT)
        buff.write_varint(val)
